...
hl.bind(const.modifier .. " + SHIFT + G", function()
	if gamemode_sm then
		...
		hl.exec_cmd(const.dirs.scripts .. "/GameMode.sh")
	end
end)
hl.bind(const.modifier .. " + ALT + L", function()
	if layout_sm then
		...
		hl.exec_cmd(const.dirs.scripts .. "/ChangeLayout.sh")
	end
end)
...
hl.bind(
	const.modifier .. " + CTRL + SHIFT + R",
	hl.dsp.exec_cmd(
		"pkill " .. launcher_cmd .. " || true && " .. const.dirs.scripts .. "/RofiThemeSelector-modified.sh"
	)
)
...
hl.bind(const.modifier .. " + N", function()
	if nightlight_sm then
		nightlight_sm:fire("toggle")
	else
		hl.exec_cmd(const.dirs.scripts .. "/Hyprsunset.sh")
	end
end)
...
hl.bind(
	const.modifier .. " + SHIFT + Return",
	hl.dsp.exec_cmd(const.dirs.scripts .. "/Dropterminal.sh " .. const.apps.terminal)
)
...
hl.bind(const.modifier .. " + ALT + mouse_down", function()
	cursor.zoom_in(2.0)
end)
hl.bind(const.modifier .. " + ALT + mouse_up", function()
	cursor.zoom_out(2.0)
end)

-- Resize (works in all layouts)
hl.bind(
	const.modifier .. " + SHIFT + left",
	hl.dsp.window.resize({ x = -50, y = 0, relative = true }),
	{ repeating = true }
)
hl.bind(
	const.modifier .. " + SHIFT + right",
	hl.dsp.window.resize({ x = 50, y = 0, relative = true }),
	{ repeating = true }
)
hl.bind(
	const.modifier .. " + SHIFT + up",
	hl.dsp.window.resize({ x = 0, y = -50, relative = true }),
	{ repeating = true }
)
hl.bind(
	const.modifier .. " + SHIFT + down",
	hl.dsp.window.resize({ x = 0, y = 50, relative = true }),
	{ repeating = true }
)
...
hl.bind(
	...
	hl.dsp.exec_cmd(const.dirs.scripts .. "/Volume.sh --inc"),
	{ locked = true, repeating = true }
)
hl.bind(
	"XF86AudioLowerVolume",
	hl.dsp.exec_cmd(const.dirs.scripts .. "/Volume.sh --dec"),
	{ locked = true, repeating = true }
)
...
hl.bind(
	"ALT_L + SHIFT_L",
	hl.dsp.exec_cmd(const.dirs.scripts .. "/SwitchKeyboardLayout.sh"),
	{ locked = true, non_consumed = true }
)
hl.bind(
	"SHIFT_L + ALT_L",
	hl.dsp.exec_cmd(const.dirs.scripts .. "/Tak0-Per-Window-Switch.sh"),
	{ locked = true, non_consumed = true }
)
