...
modify_startup_config() {
  ...
  # In new arch, startup is managed via user/startup.conf — no auto-rewrite needed.
  # Video wallpaper users should manually configure exec-once in user/startup.conf.
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm)$ ]]; then
    echo "Live wallpaper selected. Configure exec-once in user/startup.conf if needed."
  fi
}
