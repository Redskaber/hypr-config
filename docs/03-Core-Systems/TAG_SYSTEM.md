...
hl.window_rule({ size = "800 600", match = { class = "^(discord)$" } })
```
...
hl.window_rule({ match = { class = "^(discord)$" }, tag = "im" })
...
hl.window_rule({ size = "800 600", match = { tag = "im" } })
```
...
hl.window_rule({ match = { class = "^(discord)$" }, tag = "im" })
hl.window_rule({ match = { class = "^(discord)$" }, tag = "no-steal-focus" })
```
...
```bash
# sys/tags.lua
hl.window_rule({ match = { class = "^(AppClassRegex)$" }, tag = "tagname" })
```
...
hl.window_rule({ match = { class = "^([Ff]irefox|chromium|brave)$" }, tag = "browser" })
...
hl.window_rule({ opacity = "1.00 0.85".strip(), match = { tag = "browser" } })
hl.window_rule({ idle_inhibit = "fullscreen", match = { tag = "browser" } })
...
hl.window_rule({ float = true, match = { title = "negative:^(Mozilla Firefox)$" } })
```
...
```bash
hl.window_rule({ float = true, match = { tag = "im" } })
hl.window_rule({ float = false, match = { tag = "browser  # explicit tiled" } })
```
...
```bash
hl.window_rule({ center = true, match = { tag = "im" } })
hl.window_rule({ rule = "move (monitor_w*0.72) (monitor_h*0.07)", match = { tag = "pip" } })
```
...
hl.window_rule({ size = "900 600", match = { tag = "audio-mixer" } })
```
...
hl.window_rule({ opacity = "0.94 0.86".strip(), match = { tag = "im" } })
hl.window_rule({ opacity = "1.0".strip(), match = { tag = "screenshare" } })  # full opacity
```

#### 5. Visual Effects

```bash
hl.window_rule({ rule = "no_blur on", match = { tag = "multimedia-video" } })
hl.window_rule({ rule = "rounding 0", match = { tag = "games" } })
hl.window_rule({ rule = "dim_around on", match = { tag = "pip" } })
```

#### 6. Behavioral

```bash
hl.window_rule({ idle_inhibit = "always", match = { tag = "screenshare" } })
hl.window_rule({ idle_inhibit = "fullscreen", match = { tag = "browser" } })
hl.window_rule({ pin = true, match = { tag = "pip" } })
hl.window_rule({ rule = "keep_aspect_ratio on", match = { tag = "pip" } })
```

#### 7. Fullscreen

```bash
hl.window_rule({ rule = "fullscreen 0", match = { tag = "games  # borderless fullscreen" } })
```
...
hl.window_rule({ float = true, match = { title = "negative:^(.*[Tt]hunar.*)$" } })
hl.window_rule({ float = true, match = { title = "negative:^(Files)$" } })
```
...
hl.window_rule({ match = { class = "^(myapp)$" }, tag = "myapp" })
```
...
hl.window_rule({ size = "(monitor_w*0.50) (monitor_h*0.60)", match = { tag = "signal" } })

# Custom app needs full rule set
hl.window_rule({ float = true, match = { tag = "myapp" } })
hl.window_rule({ center = true, match = { tag = "myapp" } })
hl.window_rule({ size = "800 600", match = { tag = "myapp" } })
hl.window_rule({ opacity = "0.9 0.8".strip(), match = { tag = "myapp" } })
```
...
```bash
# user/tags.lua
hl.window_rule({ match = { class = "^(davinci-resolve)$" }, tag = "video-editing" })

# user/rules.lua
hl.window_rule({ opacity = "1.0".strip(), match = { tag = "video-editing" } })
hl.window_rule({ rule = "no_blur on", match = { tag = "video-editing" } })
...
```bash
# tags.conf
hl.window_rule({ match = { class = "^(myapp)$" }, tag = "mytag" })
...
TAGS_FILE="$HOME/.config/hypr/sys/tags.lua"
RULES_FILE="$HOME/.config/hypr/sys/rules.lua"
...
```
1. sys/tags.lua    → Tags assigned to windows
2. user/tags.lua   → Additional tags
3. sys/rules.lua   → Rules applied based on tags
4. user/rules.lua  → User rule overrides/additions
```
...
grep 'match:tag tagname' sys/rules.lua

# 3. Check if user rules override system rules
grep 'match:tag tagname' user/rules.lua
```
...
- [ ] Added tag in `user/tags.lua`
- [ ] Added rules in `user/rules.lua`
...
hl.window_rule({ match = { class = "^(firefox)$" }, tag = "browser" })
hl.window_rule({ match = { class = "^(discord)$" }, tag = "im" })
hl.window_rule({ match = { class = "^(thunar)$" }, tag = "file-manager" })
...
3. **Dynamic Tagging**: Auto-tag based on behavior patterns
   ...
   hl.window_rule({ rule = "auto_tag +file-dialog, maxsize 400 300" })
   ```
