...
### Lesson 15: Parameter names matter — `bezier` vs `curve`

Wiki says `hl.animation({ curve = "name" })`, not `bezier = "name"`. 45 places had it wrong. This is a silent bug — Hyprland may ignore the unknown `bezier` field and use default curve, making animations look wrong but not crashing. Only wiki verification catches this.
